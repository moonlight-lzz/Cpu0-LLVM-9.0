...
static bool supportCpu064(uint64_t Type) {
  return false;
}
...
static bool supportsCpu032(uint64_t Type) {
  switch (Type) {
  case ELF::R_CPU0_32:
...
static uint64_t resolveCpu032(RelocationRef R, uint64_t S, uint64_t A) {
  uint32_t Rel = R.getType();
  if (Rel == ELF::R_CPU0_32)
    return S + getELFAddend(R);
  return A;
}
...
getRelocationResolver(const ObjectFile &Obj) {
  if (Obj.isCOFF()) {
    ...
    case Triple::cpu0:
      return {supportsCpu032, resolveCpu032};
    case Triple::hexagon:
